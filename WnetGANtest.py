...
import time
import xlwt
# --test_rain ./TestData/input --test_ground ./TestData/label --mode test --output ./output/ --checkpoint ./model/
'''os.environ["CUDA_VISIBLE_DEVICES"] = "0"
gpu_options = tf.GPUOptions(per_process_gpu_memory_fraction=0.98)
sess = tf.Session(config=tf.ConfigProto(gpu_options=gpu_options))
'''
parser = argparse.ArgumentParser()
parser.add_argument("--test_rain", help="path to folder containing rain images")
parser.add_argument("--test_ground", help="path to folder containing ground images")
parser.add_argument("--mode", required=True, choices=["train", "test", "export"])
parser.add_argument("--output_dir", required=True, help="where to put output files")
parser.add_argument("--seed", type=int)
#parser.add_argument("--checkpoint", default=None, help="directory with checkpoint to resume training from or use for testing")
#parser.add_argument("--checkpoint", default='./output_dirain100/', help="directory with checkpoint to resume training from or use for testing")
...
def load_examples():
    if a.test_rain is None or not os.path.exists(a.test_rain):
        raise Exception("input_dir does not exist")

    input_paths = glob.glob(os.path.join(a.test_rain, "*.jpg"))
    decode = tf.image.decode_jpeg
    if len(input_paths) == 0:
        input_paths = glob.glob(os.path.join(a.test_rain, "*.png"))
...
########################
        if a.test_ground is None or not os.path.exists(a.test_ground):
            raise Exception("input_dir does not exist")

        input_paths2 = glob.glob(os.path.join(a.test_ground, "*.jpg"))
        decode = tf.image.decode_jpeg
        if len(input_paths2) == 0:
            input_paths2 = glob.glob(os.path.join(a.test_ground, "*.png"))
            decode = tf.image.decode_png
        ...
        if all(get_name(path).isdigit() for path in input_paths2):
            input_paths2 = sorted(input_paths2, key=lambda path: int(get_name(path)))
        else:
            input_paths2 = sorted(input_paths2)

        with tf.name_scope("load_images"):
            path_queue2 = tf.train.string_input_producer(input_paths2, shuffle=a.mode == "train")
            reader = tf.WholeFileReader()
            paths, contents = reader.read(path_queue2)
            raw_input2 = decode(contents)
            raw_input2 = tf.image.convert_image_dtype(raw_input2, dtype=tf.float32)

            assertion2 = tf.assert_equal(tf.shape(raw_input2)[2], 3, message="image does not have 3 channels")
            with tf.control_dependencies([assertion2]):
                raw_input2 = tf.identity(raw_input2)
...
def create_generator(generator_inputs, generator_outputs_channels):
    ...
    de_layers = []
    layer_specs_de = [
        (a.ndf * 4),  # decoder_1: [batch, 30, 20, ndf * 8] => [batch, 60, 40, ndf * 4]
        (a.ndf * 2),  # decoder_2: [batch, 60, 40, ndf * 4] => [batch, 120, 80, ndf* 2 ]
        (a.ndf),  # decoder_3:[batch, 120, 80, ndf * 2 ] => [batch, 240, 160, ndf]
        (a.ndf),  # decoder_4:[batch, 240, 160, ndf ] => [batch, 480, 320, ndf]
    ]
    for out_channels in layer_specs_de:
    ...
    with tf.variable_scope("loss"):
        x = tf.concat([de_layers[-1], layers[len(layers) - len(de_layers) - 1]], 3)
        x = tf.nn.relu(x)
        x = conv(x, 3, generator_outputs_channels, 1, name="con2_1")
        x = tf.tanh(x)
        out1 = x
    output_m = out1
    # # ------------------------------------------------------------------------------------------
    layers2 = []
    with tf.variable_scope("encoder2_1"):
        x = de_layers[-1]
        layers2.append(x)
    # layers2.append(de_layers[-1])

    for out_channels in layer_specs:
        with tf.variable_scope("encoder2_%d" % (len(layers2) + 1)):

            x = lrelu(layers2[-1], 0.2)
            x = conv(x, 3, out_channels, 1, name="con_21")
            x = batchnorm(x, name="con21_bn")
            x = lrelu(x, 0.2)
            x = conv(x, 3, out_channels, 2, name="con_22")
            x = batchnorm(x, name="con22_bn")
            layers2.append(x)

    de_layers2 = []
    for out_channels in layer_specs_de:
        with tf.variable_scope("decoder2_%d" % (len(de_layers2) + 1)):
            if len(de_layers2) == 0:
                x = layers2[-1]
            else:
                x = tf.concat([de_layers2[-1], layers[len(layers2) - len(de_layers2) - 1]], 3)
            x = tf.nn.relu(x)
            x = deconv(x, out_channels)
            x = batchnorm(x)
            de_layers2.append(x)
    ...
    de_layers.extend(de_layers2)
    return de_layers[-1], output_m, layers, de_layers
...
def create_model(inputs, targets):
    ...
    with tf.variable_scope("generator") as scope:
        ...
        outputs, outputs_m, layers, de_layers = create_generator(inputs, out_channels)
    ...
    with tf.name_scope("generator_loss"):
        ...
        # abs(targets - outputs) => 0
         ...
         loss_ssim = tf.reduce_mean((1. - tf.image.ssim(targets, outputs, max_val=1.0)) / 2.)  # SSIM loss
         gen_loss_L1 = tf.reduce_mean(tf.abs(targets - outputs)) + 0.1*tf.reduce_mean(tf.abs(targets - outputs_m)) + loss_ssim  # Additional loss function

         gen_loss = gen_loss_GAN * a.gan_weight + gen_loss_L1 * a.l1_weight # Generator loss function
...
def main():
    ...
    tf.summary.scalar("generator_loss_GAN", model.gen_loss_GAN) # 绘图函数
    tf.summary.scalar("generator_loss_L1", model.gen_loss_L1) # tf.summary.scalar(tags, values, collections=None, name=None)

    for var in tf.trainable_variables():
        tf.summary.histogram(var.op.name + "/values", var) # 直方图、用来显示训练过程中变量的分布情况
    ...
    with sv.managed_session() as sess:
        ...
        if a.mode == "test":
            ...
            table = file.add_sheet('GGAN', cell_overwrite_ok=True)
            ...
            for step in range(max_steps):
                ...
                if should(a.progress_freq):
                    ...
                    print("progress  epoch %d  step %d " % (train_epoch, train_step))
